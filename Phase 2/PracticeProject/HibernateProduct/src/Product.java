...
public class Product extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/products","root","Chandu@3");
			
			Statement st = con.createStatement();
			st.executeUpdate("use products");
			
			int res = st.executeUpdate("insert into eprod(id,name,color,price)values('"+id_int+"','"+name+"','"+color+"','"+price_cost+"')");
			ResultSet rs = st.executeQuery("select * from eprod");
